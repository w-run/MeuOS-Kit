...
| ld-as-needed | mt/ld | `--as-needed` / `--no-as-needed` | 避免不必要的 DT_NEEDED 条目（阻塞于 .so 共享库输入支持） | ⏳ | CLI 框架已就位，需 .so 输入支持 |
...
| as-full-isa | mt/as | 全架构指令完整覆盖 | 各架构缺的少用指令补全（x86_64 最完整、ARM 最小需补缺） | 🟡 | x86_64 1075 行 SSE/ALU/跳转完整；ARM 321 行需扩展 |
