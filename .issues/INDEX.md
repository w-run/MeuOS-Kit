...
| as-full-isa | mt/as | 全架构指令完整覆盖 | 各架构缺的少用指令补全（ARM 从 321→670 行，含除法/64位乘/字节反转/VFP全系列(ALU+load/store+conv+vmov)/pld/多寄存器存取） | 🟡 | ARM 编码完善（670 行，仍有 NEON 待实现）；x86_64 1075 行最完整 |
