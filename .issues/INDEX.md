...
| x86-isa-levels | **x86_64 ISA 级别门控**：实现 `-march=x86-64-v2`/`v3`/`v4` 代码生成差异。至少：v2 启用 SSE4.2+POPCNT，v3 启用 AVX2+BMI2，v4 启用 AVX-512 | 🔴 高 | 🟢 检测层（cpu_detect.c march_x86_64_v_level）+ emit 层 ISA 门控基础设施（x86_64_emit.c：`x64_features_valid()` 检查 T.features，SSE/SSE2 基线验证）。注意：mcc x86_64 后端当前仅发射 SSE 级别指令（movaps/movss/mulss/comiss/cvtss2sd），**不发射任何 AVX/AVX2/AVX-512 指令**，因此 emit 层 ISA 门控在实际代码生成中为空操作（所有发射指令均 ≤ SSE2）。当未来后端添加 VEX 编码的 AVX 指令发射器时，emit 层可直接通过 `T.features & MT_FEATURE_AVX` 做门控。 |
...
| arm-multiver | **arm 多版本后端**：根据 `-march` 切换 ARMv6/v7/v8 指令选择器和发射器差异（Thumb/ARM 模式、DMB 变体等） | 🟡 中 | 🟢 完整实现：`g_arm_arch_ver` 全局从 `-march=armvN` 提取架构版本号。`loadaddr()` 中 SGlo/SExt 地址加载：ARMv7+ → `movw`/`movt`；ARMv6 → `ldr rd, =sym`（literal pool）。`loadcon()` 常量加载：ARMv7+ → `movw`/`movt`；ARMv6 → `ldr rd, =val`。验证：`-march=armv7-a` 输出 `movw`/`movt`，`-march=armv6` 输出 `ldr =ext_arr`，均编译通过。⚠️ 此前报告的基础 bug（`unknown vector operation: {lr}`）是 MT_AS 未设置时回退到宿主 GNU as 导致的，使用 `MT_AS=mt/as` 时 ARM 编译正常。 |
| aarch64-ext | **aarch64 架构扩展**：FEAT_FP16/FEAT_RDM/FEAT_JSCVT 等特性位与代码生成 | 🟢 低 | 🟢 mcc.h 定义 MT_FEATURE_AARCH64_FP16/RDM/JSCVT 位（位 48+）；-march=armv8.2-a 解析设置扩展位。验证：armv8.2-a 可编译。emit 特性消费为后续增强。 |
...
| i386-variants | **i386 变体区分**：486/586/686 在 cmpxchg/CMPXCHG8B/FPU 存在性上的差异映射到特性位 | 🟢 低 | 🟢 mcc.h 定义 MT_FEATURE_I386_CMPXCHG/FPU/CMPXCHG8B 位；-march=i486(=CMPXCHG)/i586(+FPU)/i686(+CMPXCHG8B)。验证：i486/i586/i686 均可编译。emit 特性消费为后续增强。 |
