...
2. **提交粒度问题（需纠正）**：f8f0044 提交信息仅写「defect Q」，实际 diff 含 **Q+S+T** 三缺陷修复（cpp_parse.c 170 行 + new_delete_nullptr.cc）。R 为独立提交 93ab4b4（粒度正确）。→ 建议在 0802.md 明确记录「S/T 随 f8f0044 闭环」，并要求后续修复按缺陷独立提交，避免贡献者按提交信息误判 S/T 状态。
...
- 缺陷 U：`empty-class by-value return` 仍可能导致 m++ **编译期崩溃**（0802 记录），当前 x86_64 未验证通过；如未修应立缺陷 V。
