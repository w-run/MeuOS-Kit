...
# --- 2. 可用性判定 ---
m3_ok="no";   [ "$mm_w" != "N/A" ] && awk "BEGIN{exit !($mm_w >= 20)}" 2>/dev/null && m3_ok="yes"
...
# --- 3. 决策主模型 ---
if [ "$m3_ok" = "yes" ]; then
    main="custom-local:MiniMax-M3"; channel="minimax"; reason="MiniMax 周配额 ${mm_w}% 充足，M3 主模型"
elif [ "$cb_ok" = "yes" ]; then
    main="custom-local:deepseek-v4-flash"; channel="codebuddy-builtin"; reason="M3 不足，fallback codebuddy 自带 dsv4flash"
elif [ "$ds_ok" = "yes" ]; then
...
cat <<EOF
{
  ...
  "fallback_order": "$fb",
  "needs_switch": "$([ "$main" != "$cur_main" ] && echo yes || echo no)"
}
EOF
