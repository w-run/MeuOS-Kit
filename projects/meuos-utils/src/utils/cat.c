...
static int flag_show_all = 1;        /* 默认显示隐含"前缀" */
...
static void print_pretty_json(FILE *fp, const char *path) {
    ...
    fread(buf, 1, sz, fp);
    buf[sz] = '\0';
    json_value_t *jv = json_parse(buf, sz);
    if (jv) {
...
static void cat_one(FILE *fp, const char *path) {
    ...
    while (fgets(buf, sizeof(buf), fp)) {
        ...
        /* 语法着色 */
        if (use_color && lang != SYN_LANG_UNKNOWN && len > 0 && buf[len-1] == '\n') {
            syntax_highlight_line(lang, buf, len - 1, stdout, 1);
            fputc('\n', stdout);
        } else {
            fputs(buf, stdout);
        }
...
int main(int argc, char **argv) {
    ...
    int opt;
    while ((opt = getopt_long(argc, argv, "nbETAsh", longopts, NULL)) != -1) {
        switch (opt) {
        case 'n': flag_number = 1; break;
        case 1000: flag_number = 0; break;
        case 'b': flag_number_nonblank = 1; break;
        ...
        case 1002: flag_classic = 1; flag_number = 0; color_disable(); flag_no_color = 1; break;
