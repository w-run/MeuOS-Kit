/*
 * m4_engine.c -- minimal GNU m4-compatible macro processor (engine library)
 *
 * Extracted from meuos-buildtools m4.c.
 * Provides a reusable m4 engine as a library.
 *
...
/* --- Input string (replaces cmdline file reading) --- */
static const char *m4_input_str;
static size_t m4_input_pos;

/* --- Current file name (for __file__) --- */
static const char *m4_filename;
...
/* --- Current base input (used when no string input or include) --- */
static FILE *base_fp;
...
static void m4_putchar(int c)
{
    if (m4_out.pos < m4_out.size - 1) {
        m4_out.buf[m4_out.pos++] = (char)c;
        m4_out.buf[m4_out.pos] = '\0';
    }
}

static void m4_puts(const char *s)
{
    if (!s) return;
    while (*s) {
        if (m4_out.pos >= m4_out.size - 1) break;
        m4_out.buf[m4_out.pos++] = *s++;
    }
    m4_out.buf[m4_out.pos] = '\0';
}

static void m4_nputs(const char *s, size_t n)
{
    size_t i;
    for (i = 0; i < n; i++) {
        if (m4_out.pos >= m4_out.size - 1) break;
        m4_out.buf[m4_out.pos++] = s[i];
    }
    m4_out.buf[m4_out.pos] = '\0';
}

static void m4_printf(const char *fmt, ...)
{
    va_list ap;
    va_start(ap, fmt);
    char buf[4096];
...
static int next_char(void)
{
    ...
    /* 4. string input (replaces cmdline file reading) */
    if (m4_input_str && m4_input_str[m4_input_pos]) {
        c = (unsigned char)m4_input_str[m4_input_pos++];
        if (c == '\n')
            base_line++;
        return c;
    }

    /* 5. base FILE input (for include fallback, or stdin in m4_main) */
    while (base_fp != NULL) {
        c = fgetc(base_fp);
        if (c != EOF)
            return c;
...
static void builtin_substr(int ac, char **av)
{
    ...
    if (ac >= 3) {
        ...
        m4_printf("%.*s", n, str + from);
    } else {
        m4_printf("%s", str + from);
    }
}
...
static void builtin_translit(int ac, char **av)
{
    ...
    for (const char *p = src; *p; p++) {
        ...
        if (pos) {
            ...
            if (idx < to_len)
                m4_putchar((unsigned char)to[idx]);
        } else {
            m4_putchar((unsigned char)*p);
        }
    }
}
...
static void builtin_len(int ac, char **av)
{
    if (ac < 1) { m4_printf("0"); return; }
    m4_printf("%zu", strlen(av[0]));
}
...
static const char *rparse(const char *p, RNode **out)
{
    ...
    if (*p == '\\' && *(p+1) == '(') {
        ...
        /* bare * at top level is a literal star */
        n = rnode_alloc(R_LITERAL);
        n->ch = (unsigned char)'*';
        p++;
    } else {
...
static void builtin_patsubst(int ac, char **av)
{
    ...
    while (pos <= slen) {
        ...
        if (rmatch_find(re, str, slen, pos, &m)) {
            m4_printf("%.*s", m.start - pos, str + pos);
            ...
            case 's':
                if (argi <= ac) {
                    ...
                    if (width > 0) {
                        ...
                        if (left_just) {
                            m4_puts(val);
                            for (int i = 0; i < pad; i++) m4_putchar(pc);
                        } else {
                            for (int i = 0; i < pad; i++) m4_putchar(pc);
                            m4_puts(val);
                        }
            ...
            case 'd': {
                if (argi <= ac) {
                    ...
                    if (width > n) {
                        ...
                        if (left_just) {
                            m4_puts(buf);
                            for (int i = 0; i < pad; i++) m4_putchar(' ');
                        } else {
                            char pc = zero_pad ? '0' : ' ';
                            if (v < 0 && zero_pad) {
                                m4_putchar('-');
                                for (int i = 0; i < pad; i++) m4_putchar('0');
                                m4_puts(buf + 1);
                            } else {
                                for (int i = 0; i < pad; i++) m4_putchar(pc);
                                m4_puts(buf);
...
int m4_process(const char *input, char *output, size_t outsz)
{
    ...
    exit_code = 0;
    if (setjmp(exit_env)) {
        /* m4exit was called — stop processing */
...
{
    if (name && *name)
        macro_define(name, value ? value : "");
}

void m4_set_file(const char *filename)
{
    m4_filename = filename;
}
