...
| 编号 | 待决项 | 是否需大喵拍板 | 关联 |
|------|--------|----------------|------|
| **V1** | **guest 内投影中断对象（VIRQ）是否新增 `GUEST_INTR_INJECT`(bit56+) 显式 right，还是 Day1 复用 `VM_MANAGE`(47) 注入（立场 A）？** 本文 Day1 推荐立场 A，但嵌套虚拟化（`26 §7 V6`）场景下粒度不足，届时可能需立场 B。 | **是（架构级，需大喵拍板）** | `59 §9.3` 开放议题 5 / `07 §4.1` 48+ 预留区 / `45 §3` 分配纪律 |
| **V2** | **guest 映像度量如何并入 `.mimg` 信任链**：扩展 `.mimg` Item 携带 guest measurement 子项（`61 §3`），还是独立 `.mguest` 侧载？度量 extend 进哪个 PCR 槽？ | **是（信任链范围，需大喵拍板）** | `61 §1.2`/`§3`/`§5` + `32 §3.2`/`§3.3` + `06 §4.3` |
