...
| 编号 | 待决项 | 是否需大喵拍板 | 关联 |
|------|--------|----------------|------|
| **V1** | **guest 内投影中断对象（VIRQ）是否新增 `GUEST_INTR_INJECT`(bit56+) 显式 right，还是 Day1 复用 `VM_MANAGE`(47) 注入（立场 A）？** 本文 Day1 推荐立场 A，但嵌套虚拟化（`26 §7 V6`）场景下粒度不足，届时可能需立场 B。 | **已裁决·reasoning 决策已出（2026-08-03·待大喵复核）** | `59 §9.3` 开放议题 5 / `07 §4.1` 48+ 预留区 / `45 §3` 分配纪律 |
| **V2** | **guest 映像度量如何并入 `.mimg` 信任链**：扩展 `.mimg` Item 携带 guest measurement 子项（`61 §3`），还是独立 `.mguest` 侧载？度量 extend 进哪个 PCR 槽？ | **已裁决·reasoning 决策已出（2026-08-03·待大喵复核）** | `61 §1.2`/`§3`/`§5` + `32 §3.2`/`§3.3` + `06 §4.3` |
...
## 10. reasoning 决策（2026-08-03）

### 10.1 V1 — VIRQ 投影中断 right：Day1 复用 `VM_MANAGE`(47)（同意）

**reasoning 决策（2026-08-03）**：**采纳立场 A**——Day1 复用 `VM_MANAGE`(bit47) 注入 VIRQ，**不新增** bit 56+ `GUEST_INTR_INJECT`。立场 B（新增 `GUEST_INTR_INJECT`）作为**演进路径**明文记录，触发条件 = 嵌套虚拟化或独立中断代理子进程出现。

**被否决选项**：
- **Day1 即新增 `GUEST_INTR_INJECT`（bit56+）**：Day1 范围无细分需求，过早投入 bit 预算。
- **永不复用 `VM_MANAGE`，永久独立**：违背"Day1 务实"原则，且 bit 分配纪律严格控制。

**理由（深·reasoning 百思）**：

1. **Day1 范围内复用 `VM_MANAGE` 容量足够**。
   - Day1 范围：仅 VMM 服务（`meu-vmm` 进程）持 `VM_MANAGE`（bit47，default deny），**唯一 VMM 进程** = 唯一持权者。
   - VMM 内部对 VIRQ 注入的实现 = VMM 用户态代码（解析 host `interrupt` 对象 + 调用 `vcpu_inject` 系统动作）。
   - 核内 `vcpu_inject` 系统动作受 `VM_MANAGE` 门禁 = VMM 调用即通过。
   - **Day1 范围内无第二持权者**——复用 `VM_MANAGE` 注入无粒度问题。

2. **嵌套虚拟化（`26 §7 V6`）Day1 不支持——立场 A 在 Day1 不会显出粒度不足**。
   - `26 §7 V6` 已定：原型期一律不支持嵌套虚拟化。
   - 嵌套虚拟化场景下，VMM 内部可能有"中断代理子进程"——此时该子进程要 VIRQ 注入权 = 立场 A 下必须持 `VM_MANAGE` 全权，等于把 VM 自身管理权也下放，确实粒度不足。
   - 但 Day1 不支持嵌套虚拟化 → 该子进程不存在 → 立场 A 足够。
   - 由此：Day1 阶段立场 A 是务实选择，立场 B 仅作演进路径。

3. **`VM_MANAGE` 与 `GUEST_INTR_INJECT` 语义不同——但 Day1 范围内不构成"语义过载"**。
   - `VM_MANAGE`（bit47）= "管理 VM/设备直通委托"——VMM 对其 VM 的全权管理（含设备直通、内存映射、运行控制）。
   - `GUEST_INTR_INJECT`（若未来新增）= "向 guest 注入 VIRQ"——VMM 内部对 guest 中断的细粒度操作。
   - 严格说，二者语义不同——`59` "语义独立即拆分" 哲学会倾向独立。
   - **但 Day1 范围内**：唯一 VMM 持 `VM_MANAGE` + 无独立中断代理子 = 语义过载不会导致实际问题。
   - "Day1 务实 + 演进路径明文" = 立场 A 是正确选择。
   - 立场 B 的真正落地时机 = 嵌套虚拟化或独立中断代理子进程出现。

4. **bit 分配纪律严格控制**。
   - `45 §3` 已定：bit 0–47 冻结；bit 48+ 候选预留区按 `07` 协同定稿流程正式分配。
   - 任何新增 bit 须大喵拍板 + `07` owner 协同。
   - Day1 不抢位 = 立场 A 符合"不引入新内核原语除非必要"。

5. **VMM 用户态 VMM 内部行为 = VMM 自己的事**。
   - 投影 = VMM 软件行为（host `interrupt` 对象 → VIRQ 注入），不是新内核授权原语。
   - 内核侧 `vcpu_inject` 系统动作受 `VM_MANAGE` 门禁 = 一次 rights 检查。
   - 立场 A = 核内零新增，VMM 用户态自己负责 VIRQ 投影逻辑。

**TCB 体量评估**：
- 立场 A = 核内零新增（`vcpu_inject` 系统动作复用 `VM_MANAGE` 门禁）。
- 立场 B = 核内新增一个 right 检查点（`GUEST_INTR_INJECT` 门禁），rights 表新增 1 项。
- Day1 选 A = TCB 净增 0。

**与既有地基协同**：
- **`59 §9.3` 开放议题 5**：本决策承接其细化，与"host `interrupt` 对象模型"协同。
- **`59` "语义独立即拆分"哲学**：立场 A 在 Day1 范围内不违反（无独立中断代理子）；立场 B 是该哲学的真正落地时机。
- **`45 §3` 分配纪律**：本决策不触发 bit 56+ 分配。
- **`07 §4.1` 48+ 预留区**：`GUEST_INTR_INJECT` 仅"候选预留 bit 56+ 不分配"，立场 B 触发时按 `45 §3` 流程正式分配。
- **`00` A6 重服务出核**：VMM 是出核用户态服务，VIRQ 投影是 VMM 用户态行为，符合 A6。
- **`26 §7 V6` 嵌套虚拟化**：Day1 不支持，本决策与之一致。

**演进路径（明文）**：
- 触发条件（任一）：
  - (a) 嵌套虚拟化启用（`26 §7 V6` 由"不支持"改为"支持"）。
  - (b) VMM 内部出现独立"中断代理子进程"（如部分 VIRQ 注入职责外包给专门子进程）。
  - (c) 出现"VM 管理权与中断注入权须独立审计"需求。
- 触发后：走 `07` 协同定稿 + `45 §3` 流程正式分配 `GUEST_INTR_INJECT`（建议 bit 56），同步 `07 §4.1` rights 位图、`59` 中断子系统、`26` 虚拟化总览。
- 触发前：Day1 阶段立场 A 闭合。

**与指挥官原占位的关系**：**同意**。理由（1）（2）（4）由指挥官已覆盖；本 reasoning 决策额外补强了"立场 B 的触发条件明文化（演进路径段）"。

### 10.2 V2 — guest 度量并入 `.mimg` 信任链（同意）

**reasoning 决策（2026-08-03）**：**采纳扩展 `.mimg` Item 携带 guest measurement 子项**——guest 度量作为 `.mimg` 单容器的 Item 子项内嵌，不引入独立 `.mguest` 侧载；guest 度量 extend 进**独立 PCR 槽**（不挤占既有 PCR[0..4]）。

**被否决选项**：
- **独立 `.mguest` 侧载文件**：破坏 `.mimg` 唯一启动载体（C61-1），引入第二信任源，违背信任链单一锚原则。
- **guest 度量 extend 进既有 PCR[0..4]**：挤占 host 信任链槽位，与 `32 §3.2` 双锚冲突。
- **guest 度量完全不进信任链**：guest 映像可绕过度量 = 引入新信任源，违反 `61 §1.2` 原则 1。

**理由（深·reasoning 百思）**：

1. **保持 `.mimg` 唯一启动载体 = 信任链单一锚**。
   - `61 §1.2` 原则 1 = 信任链单一起源（`.mimg` PCR[1]），不容忍第二信任源。
   - 独立 `.mguest` = 第二信任源（即使挂到 `.mimg` 验签下，仍是"双容器"解析路径）。
   - 扩展 `.mimg` Item = 单一容器 = 单一解析路径 = 信任链单一锚不变。
   - C61-1（`.mimg` 唯一启动载体）+ C61-2（度量元数据内嵌 item）= 本决策与既有 `61` 纪律同构。

2. **信任链扩展点 = 独立 PCR 槽，不挤占既有 PCR[0..4]**。
   - 既有 PCR 分配：
     - PCR[0] 固件→loader（`32 §3.1` 跳 0）
     - PCR[1] `.mimg`（`61 §5.1` 跳 1）
     - PCR[2] monitor 自度量（`32 §3.1` 跳 2 / `61 §5.2`）
     - PCR[3] `.msys` rootfs（`32 §3.1` 跳 3 / `61 §5.3`）
     - PCR[4..] 服务（`32 §3.1` 跳 4）
   - guest 度量 = "guest 信任链" 的子链 = 不应挤占 host 信任链槽位。
   - **方案**：guest 度量 extend 进**独立 PCR 槽**（如 PCR[5+] 或专门 guest PCR 区）。
   - 由此：host PCR[0..4] 不变 + guest 度量不污染 host 信任链。

3. **单一解析路径 = 减小 TCB + 减小攻击面**。
   - 独立 `.mguest` 需新增解析/验签路径 = 双解析路径 = TCB 倍增 + 攻击面倍增。
   - 扩展 `.mimg` Item = 复用 `.mimg` 解析器 + 复用 Item 枚举 = 单一解析路径 = TCB 不增 + 攻击面不变。
   - 这是 `29 §3.2` 纵深第 0 层 + `61 §1.2` 单容器原则的最强论据。

4. **guest 映像须由已锁定公钥验签 = 与 host 度量基双锚同构**。
   - `32 §3.2` 度量基双锚（ed25519 包签名 + extent Merkle）+ `32 §3.3` 公钥根档 A/B/C。
   - guest 映像须由已锁定公钥验签，杜绝"未经验证的 guest 内核"成为新信任源（`61 §1.2` 原则 1）。
   - VMM 在 `GUEST_CREATE` 前应先完成 guest 映像验签+度量（fail-closed），与 `07` pledge/unveil 一致。

5. **与 `32` §3.2 + `61` §3 既有结构零冲突**。
   - `.mimg` 单容器格式（`61 §3`）可扩展携带 guest measurement 子项 = 不破坏四跳信任链既有结构。
   - 公钥根档 A/B/C 适用 guest 映像 = 与 host 度量同源 = 信任锚一致。
   - VMM 在 `GUEST_CREATE` 前须验签+度量 = fail-closed + 单一信任链。

**TCB 体量评估**：
- 扩展 `.mimg` Item = 解析器新增一个 Item 类型（`item_type = GuestMeasurement`），TCB 增量极小（一个枚举分支 + 一段 hash 验证）。
- 独立 `.mguest` = 新解析器 + 新验签路径 + 新度量链管理，TCB 增量显著。
- 选扩展 Item = TCB 净增小。

**与既有地基协同**：
- **`61 §1.2` 原则 1**（信任链单一起源）：本决策严格遵守。
- **`61 §3` 单容器格式**：扩展 Item 与既有结构同构。
- **`61 §5` 4 跳信任链**：guest 度量续接独立 PCR 槽，host 信任链不变。
- **`32 §3.2` 度量基双锚**：guest 映像须由已锁定公钥验签，与 host 同源。
- **`32 §3.3` 公钥根档 A/B/C**：guest 映像验签复用 host 公钥根档或继承同一来源。
- **`06 §4.3` `.msys` extent 校验和**：guest rootfs 度量复用 `.msys` Merkle（若来自 `.msys` 卷子集）。
- **`00 §5-6` 五架构同源**：度量链五架构一致（`61 §7.1`）。

**与指挥官原占位的关系**：**同意**。理由（1）（3）（5）由指挥官已覆盖；本 reasoning 决策额外补强了"独立 PCR 槽不挤占既有 PCR[0..4]（理由 2）"与"TCB 体量对比（TCB 段）"。

---

**V1 + V2 共同衔接**：
- 两位决策都建立在 `00 A6` 重服务出核 + `07 §4.5` 受约束 mint + `04` Job 账本 + `59` 中断子系统 + `61` `.mimg` 信任链 + `32` 度量基双锚既有地基之上，零新增内核原语。
- V1（VIRQ 注入权）= 走 `VM_MANAGE` 门禁；V2（guest 度量）= 扩展 `.mimg` Item + 独立 PCR 槽。
- 两者不冲突：V1 是 guest 运行时中断权；V2 是 guest 启动期信任链度量。
- 五架构同源：V1 在 host `interrupt` 对象（`59` §7 五架构一致）投影为 VIRQ；V2 在 `61` `.mimg` 信任链（`61` §7 五架构一致）扩展 Item。
