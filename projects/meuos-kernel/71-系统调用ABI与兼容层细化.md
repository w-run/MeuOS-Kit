...
| **D2** | **兼容层是否进入 TCB**：direct-mapped 整形逻辑处内核态、默认入 TCB 边界，但 emulated/synthesized 复杂语义退用户态；「兼容层整体 TCB 归属」需定调 | **是（架构级）** | `00` A3/A6、`02` §5-6 TCB 度量、`08` §5.6 |
...
| **D5** | **native/compat 双路径长期维护成本**：双路径漂移风险、五架构 × 双路径测试矩阵、direct-mapped 子集变更如何同步两套语义 | **是（架构级·维护负担）** | `08` §2.2、`21` §5、`11` §5、`19` 性能/可验证性 |
