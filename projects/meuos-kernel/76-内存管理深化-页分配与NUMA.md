...
| **P8** | **跨 node VMO 共享时物理页 node 归属与迁移是否破坏 capability 引用**（`03` §5-5 残留）：因 C1 折中已分离逻辑/物理，本文判定「不破坏」；是否需架构级确认。 | **是（架构级，建议确认本文判定）** | `03` §5-5、`00` C1 |
...
> 本文为「设计/规划」草案，结论（物理页分配器 = `03` VMO 底层供给层、NUMA 拓扑感知分配、大页/hugepage 后端、页回收/暂存与 Job 账本协同 fail-closed、device VMO 物理后端与 `60`/`10` 协同、确定性预留池与 `28` 同构、五架构薄后端同源）供执行层与 `03`/`60`/`10`/`28`/`04` 引用；待决项 P1–P7 为执行层协同定稿项，仅 P8 建议大喵架构级拍板。跨域冲突以 `00` 总览 `A1`–`A12` / `C1`–`C18` 为准。本文不修改 `00`–`70`、README、`.issues/`、git。
