...
> 此组件为 V1 骨架阶段，后续视觉优化任务已列出。设计师大喵（she/her）后续将推进：

### V2 视觉优化待办

| 任务 | 说明 |
|------|------|
| 真彩色支持 | 当前仅 16 色 ANSI，需扩展 24-bit (`\033[38;2;R;G;Bm`) |
| 字符画标题 | 用 `▀▄█` 半块字符渲染大字体 ASCII/Unicode art 标题 |
| 伪动画增强 | Spinner 增加更多样式（点线、时钟、波浪），进度条增加脉冲效果 |
| 闪烁/下划线合理运用 | 已支持 `TUI_ATTR_BLINK/UNDERLINE`，需在提示/告警/活跃状态中策略性使用 |
| 更多布局装饰字符 | 花边框 `✿` `★` `◆` `▸` `▹` 等 Unicode 装饰字符的渲染辅助 |
| 色彩主题系统 | 支持用户自定义调色板，不限于 `tui_meuos_theme` |
| 响应式布局 | 终端尺寸变化时自动重排（目前需手动再渲染） |
