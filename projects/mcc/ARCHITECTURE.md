...
- **继承**：虚继承。纯虚 `= 0` 已支持（纯虚成员/析构声明解析 + vtable 槽位留 0，派生覆写正常分派）；抽象类（含未覆写纯虚）实例化报错已支持（对象声明/`new T`/`new T[]`）；纯虚析构的完整对象生命周期仍待类外析构定义 `B::~B(){}`（该类外析构定义是既有限制）；`delete` 多态基类指针只跑基类析构（既有缺陷）。
