/* cpp_newdel_expr.c - m++ (C++) new/delete expressions.
 *
 * Stage C.3.1: split from cpp_newdel.c.  Covers the C++98 `new`/`delete`
 * expressions and their helpers (libc malloc/free wrappers, ctor
 * overload resolution):
 *   - new T / new T(args)            -> malloc + ctor call
 *   - new T[n] / new T[n]{list}      -> malloc + array ctor loop
 *   - new (place) T / new (place) T(args)  -> placement new
 *   - delete p                       -> dtor (guarded) + free
 *   - delete[] p                     -> element-wise dtor loop + free
 *
 * Helpers (static in this file):
 *   - cpp_ensure_libc_fn / cpp_malloc_expr / cpp_free_expr (libc wrappers)
 *   - cpp_ctor_expr  (Class_Class(thisp, args...) overload resolution)
 *
 * Cross-file entry points: cpp_parse_new_expr, cpp_parse_delete_expr
 * (called from expr_unary.c via local extern declarations).
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
