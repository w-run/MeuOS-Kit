...
cpp_template_decl(struct scope *s, struct type *owner)
{
	...
	/* `template <> name<args>(...) {...}` — an explicit specialization of an
	 * already-declared function template.  Since the parameter list is empty
	 * (< >), the template arguments are supplied explicitly on the function's
	 * template-id; we lower the specialization to a concrete, priority-cached
	 * instantiation so a matching call uses it instead of the primary body. */
	if (tok.kind == TGREATER) {
		next(); /* consume '>' — empty template parameter list */
		if (cpp_tok_kind() == CPP_TSTRUCT || cpp_tok_kind() == CPP_TCLASS ||
		    cpp_tok_kind() == CPP_TUNION)
			cpp_class_specialization(s);
		else
			cpp_function_specialization(s);
		return;
	}
