...
/* IR's err() was originally defined in parse.c, which mcc deleted (no text
 * IL is parsed). Without this definition the linker resolves `err` to libc's
 * err(int eval, const char *fmt, ...) — different signature — and silently
 * mis-interprets the format string as the exit code. We provide our own
 * definition that prints the message and exits 1, matching IR's intent.
 * (parse.c referenced globals `inpath` and `lnum`; we have no parser context
 * so we omit those.) */
void
err(char *s, ...)
{
