...
{
	return la64_op[op].imm && r == &i->arg[1];
}
...
storecls(int op)
{
	...
	case Ostoreb:
	case Ostoreh:
	case Ostorew: return Kw;
	case Ostorel: return Kl;
	case Ostores: return Ks;
	case Ostored: return Kd;
	default: die("invalid store opcode");
	}
}
...
fixarg(Ref *r, int k, Ins *i, Fn *fn)
{
	...
	case RTmp:
		...
		/* The address operand of every load/store is an XLEN GPR.  Its
		 * class is Kl even when the operation's data class is Kw/Ks/Kd;
		 * this is observable for VLA and aggregate addresses, which are
		 * not stack slots by the time instruction selection runs. */
		if (memarg(r, op, i) && fn->tmp[r0.val].cls == Kl)
			break;
		if (k == Kw && fn->tmp[r0.val].cls == Kl) {
			if (is32arith(op)) {
				/* LoongArch 32-bit arithmetic instructions
...
sel(Ins i, Fn *fn)
{
	...
	if (i.op != Onop) {
		...
		k1 = argcls(&i, 1);
		if (isstore(i.op)) {
			/* optab's class table uses the opcode-independent selector
			 * column for stores.  The stored value is chosen by opcode and
			 * the address is always an XLEN GPR. */
			k0 = storecls(i.op);
			k1 = Kl;
		}
		fixarg(&i0->arg[0], k0, i0, fn);
		fixarg(&i0->arg[1], k1, i0, fn);
	}
}
