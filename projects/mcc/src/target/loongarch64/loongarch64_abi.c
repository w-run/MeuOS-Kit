 ...
 *        |   |    ` gp regs passed         (0..8)
 *        |    ` fp regs passed             (0..8)
	 *        ` env pointer passed in t0        (0..1)
 */

bits
la64_retregs(Ref r, int p[2])
{
	bits b;
	int ngp, nfp;
...
selpar(Fn *fn, Ins *i0, Ins *i1)
{
	...
	cty = argsclass(i0, i1, ca, cr.class & Cptr);
	fn->reg = la64_argregs(CALL(cty), 0);
