/* i386_mabi.c — i386 cdecl ABI lowering (MIR-native, scalar core).
 *
 * Lowers the pre-ABI machine IR for scalar functions:
 *   - selpar: load arguments from the caller-pushed stack area at
 *     ebp+8 (each slot is 4 bytes wide);
 *   - selcall: push call arguments onto the stack (right-to-left,
 *     cdecl), then emit the call instruction;
 *   - selret: move the return value into EAX / xmm0.
 *
...
mabi_selpar(MFnM *fm, MOut *o, MInsM *parms, int n, uint32_t *vafa)
{
	...
	for (int i = 0; i < n; i++) {
		...
		off += 4;   /* each stack slot is 4 bytes */
	}
...
{
	(void)call;
	/* Count stack bytes needed (cdecl: all args on stack, 4 bytes each) */
	int stk = n * 4;
