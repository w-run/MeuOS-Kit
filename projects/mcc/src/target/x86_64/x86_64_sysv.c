...
{
	Ref loc, lreg, lstk, nr, r0, r1, c8, c16, c, ap;
	Blk *b0, *bstk, *breg;
	int isint;
	int offset, increment, limit;
	...
	*/
	...
	emit(Oadd, Kl, r1, ap, getcon(offset, fn));
	emit(Oadd, Kw, r0, nr, getcon(increment, fn));
	r0 = newtmp("abi", Kl, fn);
	r1 = newtmp("abi", Kl, fn);
	...
	b0->phi->to = loc;
	b0->phi->narg = 2;
	b0->phi->blk = vnew(2, sizeof b0->phi->blk[0], PFn);
	b0->phi->arg = vnew(2, sizeof b0->phi->arg[0], PFn);
	b0->phi->blk[0] = bstk;
	b0->phi->blk[1] = breg;
	b0->phi->arg[0] = lstk;
	b0->phi->arg[1] = lreg;
	...
	emit(Oadd, Kl, r0, ap, getcon(offset, fn));
}
