...
Next:
	...
	case 'u':
	case 'U':
		/* Low/high halves of a 64-bit constant.  The old direct-IR port
		 * left `>>32` in the format string, which GAS interpreted as text
		 * (and it also emitted `4+(%reg)` instead of `4(%reg)`). */
		ref = getarg(*s++, i);
		if (rtype(ref) != RCon || e->fn->con[ref.val].type != CBits)
			die("invalid 64-bit constant half");
		val = (uint64_t)e->fn->con[ref.val].bits.i;
		if (c == 'U')
			val >>= 32;
		fprintf(e->f, "$%d", (int32_t)(uint32_t)val);
		break;
	case 'H':
		sz = SShort;
		goto Ref;
	...
	case 'M':
		...
		break;
	case 'N':
		/* A memory operand advanced by one word.  Keep the displacement
		 * inside the addressing expression: `4(%rax)` and
		 * `.Ldata+4(%rip)` are both valid AT&T syntax, while prefixing a
		 * rendered %M operand is not. */
		c = *s++;
		ref = getarg(c, i);
		off.type = CBits;
		off.bits.i = 4;
		...
		case RMem:
			...
			if (rtype(m->base) == RSlot) {
				Con baseoff;
				baseoff.type = CBits;
				baseoff.bits.i = slot(m->base, e);
				addcon(&off, &baseoff, 1);
			}
			if (off.type != CUndef)
				emitcon(&off, e);
			fputc('(', e->f);
			if (rtype(m->base) == RSlot)
				fprintf(e->f, "%%%s", regtoa(e->fp, SLong));
			else if (!req(m->base, R))
				fprintf(e->f, "%%%s", regtoa(m->base.val, SLong));
			else if (off.type == CAddr)
				fprintf(e->f, "%%rip");
			if (!req(m->index, R))
				fprintf(e->f, ", %%%s, %d", regtoa(m->index.val, SLong),
					m->scale);
			fputc(')', e->f);
			break;
		case RSlot:
			fprintf(e->f, "%d(%%%s)", slot(ref, e) + 4,
				regtoa(e->fp, SLong));
			break;
		case RCon:
			off = e->fn->con[ref.val];
			...
			emitcon(&off, e);
			if (off.type == CAddr && off.sym.type != SThr)
				fprintf(e->f, "(%%rip)");
			break;
		case RTmp:
			assert(isreg(ref));
			fprintf(e->f, "4(%%%s)", regtoa(ref.val, SLong));
			break;
		default:
			die("unreachable");
		}
...
emitins(Ins i, E *e)
{
	...
	Table:
		...
		break;
	case Ostorel:
		/* x86 has no movq immediate-to-memory encoding except a sign-
		 * extended 32-bit immediate.  Split an arbitrary 64-bit constant
		 * into two word stores before falling back to the normal table. */
		if (rtype(i.arg[0]) == RCon &&
		    e->fn->con[i.arg[0].val].type == CBits) {
			emitf("movl %u0, %M1", &i, e);
			emitf("movl %U0, %N1", &i, e);
			break;
		}
		goto Table;
	case Onop:
	...
	case Ocopy:
		...
		&& e->fn->con[i.arg[0].val].type == CBits) {
			...
			/* GAS cannot encode every 64-bit immediate in a memory move.
			 * Write stack destinations as two words, including values whose
	...
	case Oaddr:
		...
		case SThr:
			if (T.pic) {
				/* local-exec relocations are rejected in a DSO.  The
				 * initial-exec GOT form is linkable for a local TLS symbol
				 * and, unlike general-dynamic TLS, needs no helper call that
				 * would invalidate allocator liveness assumptions. */
				fprintf(e->f,
					"\tmovq %s%s@gottpoff(%%rip), %%%s\n"
					"\taddq %%fs:0, %%%s\n",
					sym[0] == '"' ? "" : T.assym, sym,
					regtoa(i.to.val, SLong), regtoa(i.to.val, SLong));
				if (con->bits.i)
					fprintf(e->f, "\taddq $%"PRId64", %%%s\n",
						con->bits.i, regtoa(i.to.val, SLong));
				break;
			}
