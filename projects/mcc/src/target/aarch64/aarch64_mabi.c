/* aarch64_mabi.c — aarch64 ABI lowering (MIR-native, AAPCS64 scalar core).
 *
 * Lowers the pre-ABI machine IR for scalar integer functions:
 *   - selpar: move entry x0-x7/v0-v7 arguments into slots (overflow
 *     arguments are loaded from the caller-pushed stack area at fp+off);
 *   - selcall: move call arguments into x0-x7/v0-v7 (overflow onto the
 *     stack below sp);
 *   - selret: move the return value into x0 / v0.
 *
 * AAPCS64 difference vs riscv64: the prologue pushes fp/lr with a
 * pre-indexed stp, so the frame pointer is old_sp - 16 and caller-pushed
 * stack arguments start at fp+16 (not fp+0).
 *
 * Aggregates, varargs, floats-ops, TLS and VLA functions are rejected by
 * the aarch64 isel (mbe_supported), so this scalar core never sees them.
 */
#include <stdlib.h>
