 ...
 * currently in use
 *
 * When T.kl_in_reg == 0 (e.g., i386), Kl-class
 * temporaries are never kept in registers: they
 * are always spilled to slots. This is because the
 * ISA has no 64-bit GPRs; isel must decompose any
 * Kl operation into Kw operations on the low and
 * high halves of the Kl value's slot. Spill's job
 * here is just to assign slots to Kl temps and
 * keep Kl operands out of the live register set v.
 */
static void
limit2(BSet *b1, int k1, int k2, BSet *f)
{
	...
	bsinter(b2, mask[1]);
	if (!T.kl_in_reg) {
		/* Kl never in registers: spill all Kl temps
		 * (limit with k=0 assigns slots to them),
		 * then limit only Kw temps to the GPR budget. */
		bsinter(bkl, maskkl);
		bsdiff(b1, bkl);
		limit(bkl, 0, f);
...
store(Ref r, int s)
{
	 ...
	 * e.g. i386), the source Ref r may be a Kl temporary that has
	 * not yet been rewritten to its slot (rega runs after spill).
	 * emit's Kl dispatch for Ostorel requires the source to be
	 * slot-based, so rewrite r to its assigned slot here. This
	 * also makes the self-store (s == tmp[r.val].slot) a true
	 * no-op that emit's Ostorel Kl case detects and skips. */
	if (!T.kl_in_reg
	&& rtype(r) == RTmp && r.val >= Tmp0
	&& tmp[r.val].cls == Kl) {
		int ts = tmp[r.val].slot;
		assert(ts != -1);
		if (ts == s)
			return; /* self-store, no-op */
		r = SLOT(ts);
	}
