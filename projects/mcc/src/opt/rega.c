...
ralloctry(RMap *m, int t, int try)
{
	...
	/* On targets where Kl values never live in registers (kl_in_reg==0,
	 * e.g. i386 which has no 64-bit GPRs), Kl temporaries must always
	 * resolve to their assigned stack slot. spill.c's limit2() has
	 * already assigned a slot via slot() (tmp[t].slot != -1). Return
	 * a slot reference so the instruction is rewritten to read/write
	 * memory directly; emit's Kl dispatch then decomposes it.
	 *
	 * This check must come BEFORE the bshas(m->b, t) fast path, since
