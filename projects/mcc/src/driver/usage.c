...
"  -d<flags>          backend debug dumps (A/I/R/S/X; X=MIR pre/post, P=opt pass log, M=per-pass MIR snapshot)\n"
"  -pg                gprof profiling (accepted, no-op)\n"
"  -w                 suppress warnings\n"
"  -Wall              enable common warnings (-Wunused-variable, -Wuninitialized, ...)\n"
"  -Wextra            enable extra warnings (-Wall + -Wunused-parameter, -Wsign-compare)\n"
"  -Wunused-variable  warn about unused local variables\n"
"  -Wunused-parameter warn about unused function parameters\n"
"  -Wconversion       warn about implicit integer conversions that may truncate\n"
"  -Wsign-compare     warn about signed/unsigned comparisons\n"
"  -Wuninitialized    warn about use of possibly uninitialized variables\n"
"  -Werror / -Wno-error / -Wno-<warning>   treat warnings as errors / disable one\n"
"  --color[=auto|always|never]   colored diagnostics (default: auto = tty)\n"
"  --error-json       structured JSON diagnostics (errors + warnings)\n"
"  --error-format=FORMAT   diagnostic output: text (default) | json | sarif\n"
...
"                    -Ofast（-O3 + fast-math）\n"
"  -g                 调试信息（记录，不产出）\n"
"  -d<标志>           后端调试转储（A/I/R/S/X；X=MIR 前后快照，P=逐 pass 优化日志，M=逐 pass MIR 快照）\n"
"  -w                 抑制警告\n"
"  -Wall              启用常用警告（-Wunused-variable、-Wuninitialized 等）\n"
"  -Wextra            启用额外警告（-Wall + -Wunused-parameter、-Wsign-compare）\n"
"  -Wunused-variable  未使用的局部变量警告\n"
"  -Wunused-parameter 未使用的函数参数警告\n"
"  -Wconversion       隐式整数转换可能截断的警告\n"
"  -Wsign-compare     有符号/无符号比较警告\n"
"  -Wuninitialized    使用可能未初始化变量的警告\n"
"  -Werror / -Wno-error / -Wno-<警告>   警告视为错误 / 关闭单个警告\n"
"  --color[=auto|always|never]   彩色诊断（默认: auto = 终端自动）\n"
"  --error-json       结构化 JSON 诊断（错误 + 警告）\n"
"  --error-format=格式  诊断输出格式：text（默认）| json | sarif\n"
"  --explain          附加修复建议\n"
"\n"
"目标:\n"
