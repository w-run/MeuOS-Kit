...
### 2.3 lambda（匿名类降级）（✅ 已完成，877beed）
- **机制**（社区一致结论 + aburi 佐证）：lambda 本质是编译器合成的匿名类（closure type），成员 = 捕获变量，`operator()` = lambda 体。m++ 已有类/成员函数/运算符重载/构造函数链，**完全可复用**。**已实现**：`cpp_lambda_expr`（primaryexpr 的 `[` 入口）解析 `[captures](params) -> ret { body }`；闭包合成文件作用域匿名类 `__lambdaN`（经 cpp_class_decl 重放复用成员/构造/运算符机制）——值捕获为成员 + 合成构造（按捕获 DECLTYPE 类型化成员与 ctor 参数）、lambda 体为 `operator()`（`-> ret` 或 auto 推导）、`obj(args)` 经成员调用路径降级为 `obj.operator()(args)`（functor 含重载）；`operator()` mangle 为 `Class_operator_cl`。嵌套 lambda 支撑修复：flush_pending_methods 分离 pending 列表、cpp_parse_method_body 保存/恢复方法上下文。test/cpp/lambda.cc 10 断言（无捕获显式/auto 返回、值捕获单/多快照语义、嵌套 lambda、lambda 调 lambda）。
- **参考**：
...
- **降级策略（重要简化）**：
  1. **无捕获 lambda** → 直接生成一个 static 函数 + 指向它的函数指针，零闭包结构。✅ 实现为统一 `__lambdaN` 类（未单走 static fn 指针优化）。
  2. **按值捕获** → 匿名类成员 + 合成构造函数。✅ 已实现。
  3. **按引用捕获** → 成员为指针/引用。⛔ 未做（明确报错）。
...
- **lambda 提前到第 2-3 位** ⚠️ 偏早：lambda 语法本身不依赖 constexpr/变参模板，**前置只需 auto 或类机制**；但 lambda 捕获/闭包语义依赖引用与构造（已有）。原建议的「lambda 前先插轻量 auto/decltype 步骤」（§2.1）**已由 160e2a2 落实**（`auto f = [&]{};` 声明已解锁）。✅ **lambda 已实现**（877beed，值捕获 + functor 降级，引用捕获明确报错）。
...
3. lambda：`[&](){}` / `[=]` / 无捕获转函数指针三档测试。✅（lambda.cc 10 断言，877beed；引用捕获明确报错，默认捕获/init-capture/泛型 lambda 未做）
