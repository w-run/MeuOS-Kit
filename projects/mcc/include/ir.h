...
struct Target {
	...
	char apple;
	char windows;
	/* Set by the driver for -fPIC/-fPIE/--shared.  Targets use this when
	 * choosing relocation forms whose local-exec variants are invalid in a
	 * shared object. */
	char pic;
	...
	int kl_in_reg; /* 1 if Kl (64-bit) values may live in registers
	                ...
	                * spill.c never keeps Kl temps in v. */
