...
- `projects/mcc/src/driver/host_toolchain.c` 和 `projects/mcc/Makefile` 是 mcc-mt-integrate 集成边界，待独立提交处理。
...
- 下一阶段：as-x86_64 x86_64 assembler；mcc Makefile/driver 集成仍延后到 mcc-mt-integrate。

## 2026-07-22：as-x86_64/ld-x86_64 x86_64 核心完成
...
## 2026-07-23~27：mcc-mt-integrate-binutils 多架构扩展

- mcc-mt-integrate（mcc driver 集成）：`host_toolchain.c` 通过 `MT_AS`/`MT_LD` 环境变量集成 mt 工具链，`check-mt-integration` 验证通过。
- binutils（二进制辅助工具）：nm、readelf、strip、objcopy、objdump 全部实现，含 libelf/libdisasm 共享库。
- bootstrap-verify（自举验证）：`check-sysroot-static` 通过，sysroot 内 mcc+meow 自重建 Kit，`check-mt-integration` 零宿主 cc 依赖。
- target-i386（i386 架构）：编码器增强（ModRM/SIB/条件跳转/移位/and/or/div），ELF32 输入读取 + i386 重定位分派。
- target-aarch64（aarch64 架构）：mt/as 编码器 + mt/ld 重定位 + qemu 端到端验证通过。
- target-riscv64（riscv64 架构）：mt/as 编码器 + mt/ld 重定位 + TLS LE + qemu 端到端验证通过。
