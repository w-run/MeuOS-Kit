...
| **armhf** | **强烈建议新增** | 当前只有占位 TODO，没有 libc runtime 或构建目标 | ARMv7 hard-float EABI；同时验证 32 位指针、VFP/硬浮点 ABI、原子、TLS 与 64 位 `time_t` |
...
| **armel** | **明确跳过** | 不创建实现目标 | 极老软浮点 ARM；armhf 已覆盖仍有价值的 32 位 ARM 场景 |
...
i386 和新增的 armhf 不沿用 32 位 `long time_t`。公共头和实现应统一采用 64 位
...
5. **riscv64**：借助较干净的 ABI 抽象完成第三条 64 位完整链，反向清理公共层的
   架构泄漏。
6. **armhf**：在 64 位目标稳定后加入，重点验证 32/64 位变体和 hard-float；先以
