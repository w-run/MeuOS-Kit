...
/* riscv64 ELF TLS uses variant I with GAP_ABOVE_TP = 0 (musl riscv64 ABI):
 * the thread pointer (tp, x4) addresses the start of the TLS image directly,
 * so .tdata is copied to mmap_base + 0 and user-visible TLS reads at tp+N land
 * on .tdata[N].  The kernel programs tp via CLONE_SETTLS for new threads; the
 * main thread installs it in crt1.S via `mv tp, a0` after this returns the tp
 * base pointer (no arch_prctl-like syscall exists on riscv64). */
...
#define AT_PHNUM 5
#define PT_TLS 7
/* riscv64 asm-generic syscall numbers (identical to aarch64). */
...
allocate_tls(void)
{
	...
	result = __syscall6(RISCV64_SYS_MMAP, 0, tls_allocation_size,
 ...
 * with `mv tp, a0`; new threads get it via CLONE_SETTLS in thread_clone.S. */
void *
__meuos_tls_init(char **environment)
{
	 ...
	 * is reused later in the loop (the same bug fixed for loongarch64
	 * in commit ed49dbd).  The pointer indirection forces the compiler
	 * to keep the stack slot up to date. */
...
__meuos_tls_free(void *thread_pointer)
{
	if (thread_pointer && tls_allocation_size)
		__syscall6(RISCV64_SYS_MUNMAP, (long)thread_pointer,
