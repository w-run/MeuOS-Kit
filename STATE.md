...
10. **P10**（工具链自研，规划中）汇编器 masm / 链接器 mld / 归档器 mar + 公共库 libelf。解除 mcc 对宿主 `cc`/`as`/`ld`/`ar` 的最后依赖，完成 Kit 自举链。推荐组织：独立子项目 + 共享 `projects/libelf/`（与 libmcc 同模式）。优先级：libelf+mar (P0) → masm x86_64 (P1) → mld x86_64 静态 (P2) → 多架构扩展 (P3-6) → 动态链接 (P7) → 辅助工具 (P8)。详见 `projects/.todo/binutils-suite.md`。
...
- **2026-07-22**：工具链自研规划落地（`projects/.todo/binutils-suite.md`）——确认 mcc 通过 `host_toolchain.c` 把汇编/链接外包给宿主 `cc`，Makefile 用宿主 `ar`，这是 Kit 自举链的最后外部依赖。规划三档自研工具：第一档（核心）`mar`/`masm`/`mld`；第二档（辅助）`mnm`/`mobjdump`/`mreadelf`/`mstrip`/`mobjcopy`；第三档（扩展）`msize`/`maddr2line` 等。组织方案对比 A（单 binutils 项目）/B（完全独立）/C（独立 + 共享 libelf）三选一，推荐方案 C（与 libmcc 同模式）。优先级排序：libelf+mar (P0) → masm x86_64 (P1) → mld x86_64 静态 (P2) → 多架构 (P3-6) → 动态链接 (P7) → 辅助 (P8)。STATE.md §4 增 P10。
